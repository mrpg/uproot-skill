...
Apps are the constituent parts of projects. Create projects with `(uv run) uproot setup proj`
and apps with `(uv run) uproot new`. View `(uv run) uproot --help` if necessary. Do not create
